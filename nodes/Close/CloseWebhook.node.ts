		...
		properties: [
			{
				...
				options: [
					...
					{
						name: 'Close Account Setup',
						value: 'account_setup',
					},
				],
				default: 'lead',
				required: true,
				description: 'The object type to trigger on',
			},
			// Lead Actions
			{
				displayName: 'Actions',
	...
	webhookMethods = {
		default: {
			async checkExists(this: IHookFunctions): Promise<boolean> {
				...
				// Check if webhook still exists in Close
				try {
			...
			async create(this: IHookFunctions): Promise<boolean> {
				...
				try {
					...
					// If parameter doesn't exist, default to common actions
					selectedActions = ['created'];
				}

				// Build events array based on trigger type and actions
...
/**
 * Build the events array for webhook registration based on trigger type and selected actions
 */
function buildEventsArray(triggerOn: string, actions: string[]): Array<{ object_type: string; action: string }> {
	...
	switch (triggerOn) {
		...
		case 'bulk_action':
			for (const action of actions) {
				// Bulk actions can have multiple types
...
/**
 * Map account setup actions to their corresponding object_type and action
 */
